...
// loadOrCreateHamsfile reads the git-config.hams.yaml (or its
...
// hamsfilePath returns the absolute path to git-config's hamsfile (or
// its .local.yaml variant) for the currently active profile.
func (p *ConfigProvider) hamsfilePath(hamsFlags map[string]string, flags *provider.GlobalFlags) (string, error) {
