...
- [ ] 1.1 Create `internal/provider/baseprovider/baseprovider.go` with
...
- [ ] 1.5 Verify `task test:unit` passes.
- [ ] 1.6 Atomic commit: `feat(provider): add baseprovider package with
  EffectiveConfig/HamsfilePath/LoadOrCreateHamsfile`.

## 2. Passthrough primitive

- [ ] 2.1 Create `internal/provider/passthrough.go`:
...
- [ ] 2.2 Create `internal/provider/passthrough_test.go` swapping
  `PassthroughExec` to record invocations; covers DryRun-no-exec
  case + normal-pass-through case + nil-flags safety.
- [ ] 2.3 Migrate every `WrapExecPassthrough` call site to
  `Passthrough`: homebrew (default branch), cargo (default), apt
  (default), git/unified.
- [ ] 2.4 Delete `internal/provider/wrap.go`'s `WrapExecPassthrough`
  function (and its test if any). Project rule: no compat shim.
- [ ] 2.5 `task test:unit` passes; integration tests untouched (they
  exercise the real exec path).
- [ ] 2.6 Atomic commit: `feat(provider): add Passthrough primitive
  with DryRun support and DI seam`.

## 3. storeinit fine-grained DI seams

- [ ] 3.1 Replace `var ExecGitInit = defaultExecGitInit` and
...
- [ ] 3.2 Refactor `defaultExecGitInit` body to use the new seams.
  Keep `defaultGoGitInit` as the go-git fallback (uses
  `go-git/v5`'s `git.PlainInit`).
- [ ] 3.3 Update `internal/storeinit/storeinit_test.go` to swap the
...
- [ ] 3.4 `task test:unit` passes; **apt E0 integration test runs
  unchanged** and still asserts the "bundled go-git" stderr line.
- [ ] 3.5 Atomic commit: `refactor(storeinit): replace coarse function
  seams with fine-grained LookPathGit/ExecCommandContext/GitInitTimeout`.

## 4. Spec updates and verification

- [ ] 4.1 Write `specs/provider-system/spec.md` delta with SHALLs:
  "Provider passthrough flow MUST honour `--dry-run`",
  "Package-class providers SHOULD use `baseprovider.*` helpers
  for hamsfile/config resolution".
- [ ] 4.2 Write `specs/cli-architecture/spec.md` delta with SHALL:
...
- [ ] 4.3 `task check` passes end-to-end (fmt + lint + unit +
  integration + e2e).
- [ ] 4.4 Run `/opsx:archive 2026-04-19-shared-abstraction-fanout`.
