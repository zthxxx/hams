...
hams ships with 14 builtin providers that cover the bulk of what sits on a typical workstation.
...
| [code](./code) | `code` | macOS, Linux | VS Code extensions |
| [mas](./mas) | `mas` | macOS only | Mac App Store apps |
| [git](./git) | `git` | macOS, Linux | Global git config + repo clones |
...
Names in `provider_priority` match the internal registry names, which is why `git` appears here
