...
export HAMS_STORE=/tmp/test-code-store
export HAMS_MACHINE_ID=e2e-code
export HAMS_CONFIG_HOME=/tmp/test-code-config
...
# Provider Name=code and FilePrefix=code both match the CLI verb after the
# 2026-04-18 rename. Two small well-maintained extensions:
# `vscode-icons-team.vscode-icons` is ubiquitous;
# `tamasfe.even-better-toml` succeeded the deprecated
# bungcip.better-toml.
