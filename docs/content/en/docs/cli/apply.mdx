...
# Fresh machine, one command: clone the store, pick the macOS profile,
# and apply. Auto-creates ~/.config/hams/hams.config.yaml when missing.
hams apply --from-repo=zthxxx/hams-store --tag=macOS

# Same, but also auto-run the provider bootstrap script when something
# is missing (e.g. Homebrew). hams asks first on a TTY; --bootstrap
# pre-confirms.
...
# Switch profile for a single run without touching the global config
...
| `--tag` | string | Active profile tag. `--profile` is a legacy alias for the same slot. Precedence: `--tag` > config `tag:` > `"default"`. Passing both flags with *different* values exits with usage error `2` |
...
distro), `code` needs a GUI app (VS Code), `apt`/`defaults` are platform-gated, `git` is
pre-present on any machine that ran hams's installer. For these, hams emits a plain "not found in
PATH" error with the install hint.
