...
Not every provider supports every verb. `hams git clone` accepts the natural git syntax
(`hams git clone <url> <path>` — internally maps onto the `add` verb of the underlying git-clone
provider); `bash` uses `run`. Check the individual provider doc for specifics.
