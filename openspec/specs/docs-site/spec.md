...
Define the requirements for the hams documentation site, hosted on GitHub Pages at `hams.zthxxx.me`, so that users, contributors, and provider authors have a single authoritative reference with locale-prefixed routing, consistent navigation, and client-side language switching.
