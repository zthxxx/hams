...
A holistic outside code-review at session end (superpowers code-reviewer) confirmed: NO ship-blockers. The work is correct on every path the user will touch. Three POST-MERGE NITs noted for future cleanup, all sharing one root cause:

1. **State-pin clear on remove**: `hams apt install nginx=1.24.0` then `hams apt remove nginx` leaves `requested_version: 1.24.0` stale in the StateRemoved row. State YAML lies about intent. Fix: pass `WithRequestedVersion("")`+`WithRequestedSource("")` in handleRemove's SetResource call.
2. **Remove-with-pin keys orphan state row**: `hams apt remove nginx=1.24.0` writes state under `nginx=1.24.0` instead of bare `nginx`. Fix: `parseAptInstallToken(arg)` in handleRemove to recover the bare name.
3. **Hand-edit unpin leaves stale pin in state**: hamsfile edit `{app: nginx, version: "1.24.0"}` → `{app: nginx}` + `apply` runs `apt-get install nginx` correctly but state's `requested_version` doesn't get cleared. Fix: in apt.Plan's Skip branch, attach `WithRequestedVersion("")`+`WithRequestedSource("")` when the hamsfile pin is absent.
