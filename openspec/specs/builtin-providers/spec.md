...
1. **Bare name** — `hams apt install nginx` records `{app: nginx}` and probes `dpkg -s nginx` for the observed version.
2. **Version pinned** — `hams apt install nginx=1.24.0` records `{app: nginx, version: "1.24.0"}` and probes `dpkg -s nginx`. The recorded `version` is the user's REQUESTED pin; the observed version goes into state's `version` field. Plan SHALL re-install when observed != requested.
