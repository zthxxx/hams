...
- [x] 8.1 Add `internal/provider/filter.go::HasArtifacts(p, profileDir, stateDir) bool` — returns true if `<profile>/<FilePrefix>.hams.yaml`, its `.local.yaml` sibling, or `.state/<machine>/<FilePrefix>.state.yaml` exists.
- [x] 8.2 Modify `internal/cli/commands.go::runRefresh` to filter registry by `HasArtifacts` BEFORE applying `--only`/`--except`. Debug-log skipped providers.
- [x] 8.3 Modify `internal/cli/apply.go` (or its equivalent) to apply the same two-stage filter before `ProbeAll`, `Plan`, and `Execute`.
- [x] 8.4 Unit tests in `internal/provider/filter_test.go` — empty profile + empty state → skipped; hamsfile only → included; state only → included; both → included; `--only` narrows within the stage-1 result; `--only` outside stage-1 result → empty-result no-op.
- [x] 8.5 Unit tests in `internal/cli/*_test.go` — `runApply` / `runRefresh` with mixed providers: only providers with artifacts are probed.

## 9. Per-provider integration test scaffolding (new test infra)

- [x] 9.1 Create `e2e/base/Dockerfile` — `FROM debian:bookworm-slim` + `ca-certs`, `curl`, `bash`, `git`, `sudo`, `yq` (from GitHub release, pinned version). No language runtimes.
- [x] 9.2 Relocate `e2e/lib/*.sh` → `e2e/base/lib/*.sh`. Update callers in `e2e/debian/`, `e2e/alpine/`, `e2e/openwrt/`, `e2e/sudo/`, `e2e/integration/`.
- [x] 9.3 Add `e2e/base/lib/provider_flow.sh` — exports `standard_cli_flow <provider> <install_verb> <existing_pkg> <new_pkg>` implementing the canonical 5-step flow (seed-install existing → sleep → re-install existing → install new → refresh → remove new) with state-file assertions after each step.
- [x] 9.4 Add Taskfile entries `ci:itest:base` (build base image if missing), `ci:itest:run PROVIDER=<name>` (build provider image + run integration.sh), `ci:itest` (loop all in-scope providers), `test:itest:one PROVIDER=<name>` (same via `act`).
- [x] 9.5 Add `.github/workflows/ci.yml` matrix job `itest` with one entry per in-scope provider, each invoking `task ci:itest:run PROVIDER=<name>`.

## 10. Migrate apt to new per-provider integration layout

- [x] 10.1 Create `internal/provider/builtin/apt/integration/Dockerfile` — `FROM hams-itest-base:<base-hash>` with no delta (apt-get already in base).
- [x] 10.2 Create `internal/provider/builtin/apt/integration/integration.sh` — sources shared helpers, runs `apt-get update`, then calls `standard_cli_flow apt install jq btop` + apt-specific extras (E1–E3 install/remove cycle, schema v1→v2 migration E5 moved from `e2e/debian/assert-apt-imperative.sh`).
- [x] 10.3 Prune `e2e/debian/assert-apt-imperative.sh` so it keeps ONLY the store-level profile_tag rejection E4 (cross-provider config concern), or merge E4 into a new `e2e/debian/assert-config-scope.sh`. Update `e2e/debian/run-tests.sh` to drop the `run_apt_imperative_tests` + `run_apt_cli_only_flow` calls — apt integration now lives in its own container.
- [x] 10.4 Update `.claude/rules/development-process.md` — the "Standardized provider integration test process" section now points at `internal/provider/builtin/<provider>/integration/integration.sh` with `standard_cli_flow` as the reference helper; the apt path is the canonical example.
...
- [x] 11.3 cargo — Dockerfile installs rustup + stable toolchain + gcc; integration.sh runs `standard_cli_flow cargo install tokei just`.
...
- [x] 11.8 npm — Dockerfile installs Node.js 20 via NodeSource repo; integration.sh runs `standard_cli_flow npm install serve nodemon`.
- [x] 11.9 pnpm — Dockerfile installs Node.js 20 + activates pnpm via `corepack`; integration.sh runs `standard_cli_flow pnpm add serve nodemon`.
- [x] 11.10 uv — Dockerfile installs uv via the official install script; integration.sh runs `standard_cli_flow uv install ruff httpie`.
- [x] 11.11 vscodeext (code-ext) — Dockerfile installs the VS Code CLI standalone binary; integration.sh exports `STATE_FILE_PREFIX=vscodeext` (since Manifest Name=`code-ext` differs from FilePrefix=`vscodeext`) and a `POST_INSTALL_CHECK` hook using `code --list-extensions`, then runs `standard_cli_flow code-ext install vscode-icons-team.vscode-icons tamasfe.even-better-toml`.
...
- [x] 12.2 `/opsx:archive fix-apt-cli-state-write-and-htop-rename` — archived 2026-04-15 with `--skip-specs`; deltas applied to main specs manually (builtin-providers, cli-architecture, dev-sandbox, schema-design); all gates green at archive time (`task check` + `task ci:itest`).
