// Package baseprovider hosts the cross-provider helpers that every
// builtin CLI-first auto-record provider duplicates. Pre-baseprovider,
// each of the 13 builtin providers shipped a hand-written hamsfile.go
// with the same loadOrCreateHamsfile, hamsfilePath, and effectiveConfig
// helpers — only the package name and the FilePrefix differing.
// Centralizing the helpers shrinks the per-provider boilerplate to one
// file (the package's runner + Provider struct + Manifest) and locks
// the contract: every provider's hamsfile resolution rules stay in
// lockstep.
//
...
// helpers operate on the values they need (cfg + filePrefix + flags)
// so adopting them does not require any structural refactor of
// existing providers.
...
// profile, creating an empty document when the file does not yet
// exist. filePrefix is the provider's Manifest().FilePrefix.
...
// (or its .hams.local.yaml variant when --hams-local was passed) for
