...
| File | `code.hams.yaml` (Manifest.Name `code` = FilePrefix `code`; the pre-rename `code-ext` / `vscodeext.hams.yaml` were renamed in the git/code unification pass). |
| Resource class | Package |
| Platform | both |
...
| Any other | Not passthrough (`code` in hams only exposes the extension-management verbs; the VSCode CLI itself has no generic passthrough surface here). |
