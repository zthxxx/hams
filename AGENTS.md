...
- [x] **6. Final verification pass** (`CLAUDE.md` → *Mandatory Verification Before Delivery*)
  ...
  - [x] 6.5 **8 of 9 adopters** — `cargo`, `npm`, `pnpm`, `uv`, `mas`, `vscodeext`, `goinstall` use `AutoRecordInstall` / `AutoRecordRemove`; `homebrew` uses the closure variants `AutoRecordInstallFn` / `AutoRecordRemoveFn` (shipped via follow-up 5.8) to curry its `isCask` flag and tap-vs-uninstall routing. The only remaining non-adopter is `apt` — its complex per-pkg structured-field recording + apt-get simulate-flag detection + post-install `dpkg -s` probe exceed what a closure can express; a richer dispatcher variant is tracked as archive follow-up 5.7.

- [x] **7. Absorb origin/dev's infrastructure wins (cycle 2026-04-19)** — three OpenSpec changes, all archived after `task check` exited 0 with `=== All OpenWrt E2E tests passed ===`.
  - [x] 7.1 `archive/2026-04-19-shared-abstraction-fanout/` — new `internal/provider/baseprovider/` package centralises `EffectiveConfig` / `HamsfilePath` / `LoadOrCreateHamsfile` (13 providers migrated, ~90 lines of duplication removed); new `internal/provider/passthrough.go` with `PassthroughExec` DI seam + `flags.DryRun` honouring (9 `WrapExecPassthrough` call sites migrated, function deleted — no compat shim per project rule); `internal/storeinit/` gains fine-grained `LookPathGit` / `ExecCommandContext` / `GitInitTimeout` package vars on top of the existing function-level seams. New tests: `baseprovider_test.go` (7 cases), `passthrough_test.go` (4 cases), `seams_test.go` (3 cases). Spec deltas merged into `provider-system/spec.md` + `cli-architecture/spec.md`.
  - [x] 7.2 `archive/2026-04-19-cli-modularization/` — new `internal/cli/autoinit.go` extracts `ensureProfileConfigured`, `statFile`, and the new `enforceTagProfileConsistency` helper out of the 1559-line `apply.go`; `apply_autoinit_test.go` renamed to `autoinit_test.go` and gains 8 new direct unit tests covering CLI-tag seed, no-op-when-config-exists, non-TTY error, --tag/--profile alias equivalence, conflict rejection, and `statFile` branches; `enforceTagProfileConsistency(flags)` wired into 7 additional CLI entry points (refresh, list, config list, config get, store status, store init, store push, store pull) so `--tag X --profile Y` rejects everywhere `apply` already did. `WarnIfDefaultsUsed` extraction (commit `3cafb27`) preserved — `--help` / `--version` stay fully silent. Spec deltas merged into `cli-architecture/spec.md`.
  - [x] 7.3 `archive/2026-04-19-i18n-and-log-assertion-expansion/` — i18n catalog expanded from 118 to 293 typed constants by porting 175 new keys verbatim from `origin/dev` (autoinit, ufe, expanded apply/refresh, store, config, list, upgrade, sudo, TUI, bootstrap clone, errors prefix, provider help, git dispatcher, expanded CLI errors). Both `en.yaml` and `zh-CN.yaml` gain matching entries — `TestLocalesAreInParity` confirms lockstep. New `catalog_coherence_test.go` hand-maintains a slice of every typed constant and asserts each resolves in both locales; coexists with the existing dynamic-parity and template-interpolation tests for best-of-three coverage. New `assert_log_contains` and `assert_log_records_session` helpers in `e2e/base/lib/assertions.sh` complement the stderr-based gate already in `standard_cli_flow`; apt's `integration.sh` becomes the canonical reference (sets `HAMS_DATA_HOME=/tmp/test-apt-data` for log isolation, fires `apply --only=apt` post-canonical-flow, asserts `assert_log_records_session` + `assert_log_contains "apt"`). Spec deltas merged into `cli-architecture/spec.md` + `code-standards/spec.md`. **Wiring of the 175 new constants into `internal/cli/**` call sites is deferred to follow-up work** — the catalog is in place and importable today; the rewrite is mechanical refactor in its own right.
  - [x] 7.4 Source: input came from `docs/notes/dev-vs-loop-implementation-analysis.md` §9 (the gap analysis written in commit `91e1dfc`). The complementary §10 list — what `origin/dev` would absorb FROM `local/loop` — remains canonical reference material if the inverse merge ever happens.
