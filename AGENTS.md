...
### Cycle 242 — `--debug` honored by per-provider CLI commands

- [x] Pre-cycle-242 only `hams apply` and `hams refresh` applied `flags.Debug` to the global slog level (via `logging.Setup`). Per-provider CLI commands (`hams cargo install foo --debug`, `hams brew remove bar --debug`, etc.) parsed `--debug` into `flags.Debug` but never raised the slog level — debug-level calls in wrap.go ("executing wrapped command"), homebrew ("listing taps failed"), and 11 other production sites were silently dropped. New `logging.SetupDebugOnly(debug bool)` helper: routes slog to stderr only (no per-invocation log file — short commands don't justify file rotation), sets level by debug flag. Wired into `routeToProvider` so every CLI dispatch path picks up the level. apply/refresh keep using full `Setup` with file rotation. Test `TestSetupDebugOnly_DebugLevelEmitsDebugLines` captures stderr around (Debug + Info) calls under both `debug=true/false` and asserts the level filter. (commit `4aca2ec`)

### Cycle 241 — `hams apply --dry-run` skips pre-apply refresh state writes

- [x] Symmetric with cycle 226 (runRefresh). The pre-apply refresh inside runApply was unconditionally calling `sf.Save` on every probed provider, so `hams apply --dry-run` on a fresh store CREATED `.state/<machine>/<provider>.state.yaml` files for every probe-succeeded provider — surprising users debugging "why does dry-run leave a state directory behind?" and contradicting the global flag's "no changes" contract. Add `if flags.DryRun { continue }` to the save loop; probe still runs (dry-run preview needs the result) but the in-memory state is discarded. Pre-apply-save-failure-ordering test was rewritten to use non-dry-run mode with an empty planFn since dry-run no longer reaches the save path. (commit `66c5055`)

### Cycle 240 — `hams --json --dry-run apply` emits `elapsed_ms` (schema parity with real-run)

- [x] Cycle 238 added `elapsed_ms` to apply's real-run JSON; the dry-run path (`emitDryRunJSON`) was missed, so the two JSON shapes diverged. CI dashboards diffing dry-run previews against real applies had to special-case the field as "may be missing on the dry-run path". Thread `time.Since(applyStart).Milliseconds()` through to `emitDryRunJSON` (signature gained `elapsedMs int64`). Both apply JSON paths now share the same field name + units. Test `TestRunApply_DryRunJSONHasNoProse` now asserts elapsed_ms is present and non-negative. (commit `237a0a8`)

### Cycle 239 — Every refresh/apply text completion branch shows elapsed time

- [x] Cycle 238 wired `(took Xms)` into happy-path summaries; cycle 239 closes the asymmetry across the remaining text branches: refresh interrupted-ctx, refresh save-failures-only, refresh probe-failures, apply dry-run "No changes made". Users see consistent timing regardless of how the run ended — useful for "did the probe hang?" debugging on Ctrl+C and CI alerting on regression for partial-failure runs. The save-failure-alphabetical regression test (`TestRunRefresh_SaveFailureListIsAlphabetical`) was updated to strip the new `(took Xms)` suffix before its byte-for-byte comparison so the deterministic ordering invariant stays intact while accommodating the non-deterministic timing. (commit `772f458`)

### Cycle 238 — `hams apply` + `hams refresh` surface elapsed wall-clock time

- [x] CI / observability gap. Pre-cycle-238 only the OTel exporter knew apply/refresh durations (`hams.probe.duration` metric, opt-in via HAMS_OTEL). Users debugging slow runs had to scrape slog timestamps; CI scripts couldn't alert on regression. Capture wall-clock start at the top of each Action and emit elapsed milliseconds on every exit path: text summary appends `(took Xms)`; JSON summary adds `"elapsed_ms": <int64>`. Both `runApply` (real-run + interrupted-ctx implicit via defer) and `runRefresh` (completion + interrupted-ctx + dry-run-skip-saves branches). Symmetric across the two commands. The OTel metric stays — it's a per-phase sub-measurement; `elapsed_ms` is the user-facing total. Tests `TestRunApply_JSONOutput` + `TestRunRefresh_JSONOutput` now require the field and assert non-negative. (commit `3fa2f9b`)

### Cycle 237 — OTel trace/metric filenames include nano + PID (no silent collisions)

- [x] Subtle silent-data-loss bug in the opt-in OTel local file exporter. Pre-cycle-237 `writeJSON` named files with per-second timestamps (`YYYYMMDDTHHmmss.json`). Two `hams apply`/`hams refresh` runs completing within the same wall-clock second had the second writer's `os.Create` truncate the first writer's file — silent loss of trace data. Granted: HAMS_OTEL is opt-in and refresh typically takes >1s, but the failure mode was data-corrupting and invisible to the user. Filename now `YYYYMMDDTHHmmss.NNNNNNNNN.PID.json` (nano resolution + PID handles same-second concurrent writes). Regression test `TestFlush_RapidFlushesProduceUniqueFilenames` runs 5 back-to-back flushes through LocalFileExporter and asserts 5 distinct files (pre-fix produced 1). (commit `8db6326`)

### Cycle 236 — `hams --json config get` emits structured `{key, value, set}` object

- [x] Pre-cycle-236 `hams --json config get profile_tag` ignored the global `--json` flag and produced plain text — CI scripts had to special-case `config get` while every other command honored `--json`. Worse, the text mode couldn't distinguish "set to empty string" from "unset" (both produced an empty stdout line). Refactor `printConfigKey` into `printConfigKeyMode(..., jsonMode)`: when JSON, emit `{"key": "...", "value": "...", "set": <bool>}`; when text, preserve the legacy contract (no output for unset keys so `[[ -n "$(hams config get X)" ]]` idiom keeps working). New `TestPrintConfigKeyMode_JSONShape` table-drives 4 representative cases (set/unset profile_tag/machine_id) and asserts the JSON shape + set semantics. (commit `e8eafea`)

### Cycle 235 — `hams apply` text output names failed providers

- [x] Symmetric with cycle 231's JSON `failed_providers` and cycle 234's refresh text naming. Pre-cycle-235 the apply prose said `hams apply complete: ... %d failed` (count only) — interactive users had to grep slog to find WHICH providers failed. The adjacent skipped/state-save warnings already named providers; this closes the asymmetry. New line: `Warning: %d provider(s) had failed actions: alpha, beta` + a follow-up `Re-run with --debug ...` hint. Names sorted for stability. Regression test `TestRunApply_TextOutput_NamesFailedProviders` seeds alpha (succeeds) + beta (applyFn errors); asserts stdout names "beta" with the new phrase. (commit `8c29356`)

### Cycle 234 — `hams refresh` text output names probe-failed providers

- [x] Symmetric with cycle 232's JSON list. Pre-cycle-234 the prose summary for partial probe failures said `(N probe error(s); see log for details)` without naming the failing providers. Interactive users had to grep slog. Now: `Refresh complete: X/N providers probed (M probe error(s) in: alpha, beta; see log for details)`. Same set-difference logic as the JSON path; sorted for stability. New regression test `TestRunRefresh_TextOutput_NamesProbeFailedProviders` seeds alpha (succeeds) + beta (probe errors) and asserts the stdout summary mentions "beta" inline. (commit `f2c857b`)

### Cycle 233 — `hams apply` upfront sudo prompt fires for bash hamsfiles with `sudo: true` entries

- [x] Cycle 227 added `Manifest.RequiresSudo` to gate `sudoAcq.Acquire`. But bash can't declare sudo statically — each hamsfile entry optionally sets `sudo: true`, knowable only after parsing the file. Pre-cycle-233 a bash-only profile with `sudo: true` scripts silently skipped the upfront acquire and then prompted for password EACH TIME a sudo script ran during Execute. On long applies the sudo ticket from the first script could time out before the next ran. New helper `bash.HamsfileHasSudoEntries(path)` parses the bash hamsfile and reports whether any entry has `sudo: true` (fail-open on read/parse errors — false positive harmless, false negative resurrects the multi-prompt regression). `apply.providersNeedSudo` gained a `profileDir` parameter and now treats bash as sudo-needing when its hamsfile signals sudo. Effect: upfront acquire fires ONCE for bash-with-sudo profiles, same UX as apt. (commit `06b2063`)

### Cycle 232 — `hams --json refresh` names probe-failed providers

- [x] Symmetric to cycle 231's apply fix. Pre-cycle-232 the refresh JSON exposed `"probe_failures": N` (count only) but didn't name WHICH providers failed to probe. CI retry scripts (`hams refresh --only=$(jq -r '.probe_failed_providers | join(",")')`) had no programmatic source for the list. Compute the set-difference of attempted providers minus succeeded `probeResults` keys, sort, emit as `probe_failed_providers`. Empty array on happy path. Tests: existing `TestRunRefresh_JSONOutput` requires the field; new `TestRunRefresh_JSONOutput_NamesProbeFailedProviders` seeds alpha (succeeds) + beta (probeFn returns error) and asserts `probe_failed_providers = ["beta"]`. (commit `8764955`)

### Cycle 231 — `hams --json apply` names failed providers in summary

- [x] CI-ergonomics gap. Pre-cycle-231 the apply JSON summary said `"failed": N` (count only) but didn't name WHICH providers failed. A retry-failed-only script (`hams apply --only=$(jq -r '.failed_providers | join(",")')`) had no programmatic source for the list — had to grep slog output. Track per-provider failure attribution in the apply loop (`if result.Failed > 0 { failedProviders = append(...) }`) and emit `failed_providers` (list of provider names) in the JSON summary, mirroring the existing `skipped_providers` and `state_save_errors` arrays. Empty array on happy path; nil-safe normalization. New regression test `TestRunApply_JSONOutput_FailedProvidersNamesFailures` (seeds alpha+beta, beta's applyFn errors, asserts `failed_providers = ["beta"]`); existing `TestRunApply_JSONOutput` updated to require the field. (commit `2e12518`)

### Cycle 230 — `hams --json apply` emits `dry_run` flag in real-run summary

- [x] Schema-consistency follow-up to cycle 229. Apply's real-run JSON output omitted `dry_run` while `emitDryRunJSON` (the dry-run path) included `dry_run: true`. CI consumers had to check field presence to tell modes apart — brittle. Now apply's normal-completion path emits `dry_run: false`. Together with cycle 229, both apply and refresh share the same convention: `dry_run` is always present; value indicates the mode. `TestRunApply_JSONOutput` updated to require the field and assert default-false. (commit `eb9a498`)

### Cycle 229 — `hams --json refresh` emits `dry_run` flag

- [x] Cycle 226 promised this in its commit message ("the JSON summary further down already reports `dry_run: true`") but the field wasn't actually in the output. CI scripts running `hams --json refresh --dry-run` to preview drift had no programmatic way to tell apart a real refresh from a dry-run preview — they had to re-parse argv. Add `dry_run` (mirroring `flags.DryRun`) to both JSON branches (normal completion + interrupted-ctx). Two test changes: `TestRunRefresh_JSONOutput` now requires `dry_run` in the shape and asserts default-false on real runs; `TestRunRefresh_JSONOutput_DryRunFlagSurfacesTrue` (new) asserts the true case. (commit `cce4a24`)

### Cycle 228 — `runApply` exits with ExitSudoError on sudo cancel/timeout

- [x] Closes the symmetric gap from cycle 227. cli-architecture/spec.md §"Sudo not granted" mandates exit code 10 (`ExitSudoError`) when the user cancels the sudo prompt or sudo times out. Pre-cycle-228 the failure was downgraded to a `slog.Warn` and apply continued — apt's later `runner.Install` would fail with a generic provider error, the user got the wrong exit code, and CI scripts couldn't distinguish "user canceled" from "apt-get returned 100". Now: when `providersNeedSudo` is true AND `sudoAcq.Acquire` errors, return `ExitSudoError` with three actionable suggestions (re-enter password, configure NOPASSWD sudo, or `--except=apt`). Regression test `TestRunApply_SudoFailureExitsWithSudoError` uses an inline `failingAcquirer` and asserts the error code, message, and that Plan is NOT called (short-circuit). (commit `2a215eb`)

### Cycle 227 — `hams apply` skips sudo prompt when no provider needs it

- [x] Real spec violation. cli-architecture/spec.md §"Sudo management" mandates "Operations that do not require sudo SHALL NOT prompt for credentials." Pre-cycle-227 `runApply` called `sudoAcq.Acquire` on every invocation — a user whose profile contained only cargo/npm/brew/pnpm/uv/git-clone (none of which need sudo) still got a sudo password prompt at the start of every `hams apply`. Three-part fix: (1) add `RequiresSudo bool` to `provider.Manifest` (default false; per-provider opt-in). (2) Set `RequiresSudo: true` in apt's manifest (the only v1 builtin whose runner shells out as root). (3) Add `providersNeedSudo` helper in apply.go and gate `sudoAcq.Acquire` on it. The check runs after dry-run + filter so `--only=cargo` on an apt+cargo profile correctly suppresses the prompt. Two new regression tests (`TestRunApply_NoSudoPromptWhenNoProviderRequiresIt` for the cargo-only case, `TestRunApply_SudoPromptWhenAptIncluded` for the apt case); existing `TestRunApply_UsesFilePrefixStatePathAndProviderPlan` updated to set `RequiresSudo=true` on its seed manifest. (commit `20fbe6f`)

### Cycle 226 — `hams refresh --dry-run` is a pure preview (no state writes)

- [x] Real --dry-run contract violation. Pre-cycle-226 `runRefresh` probed every provider (read-only, fine) and then unconditionally called `sf.Save` on every result — writing state files and bumping `first_install_at`/`updated_at` timestamps despite the user's explicit `--dry-run`. CI scripts running `hams refresh --dry-run` to check planned drift were silently mutating state. Gate the save loop on `!flags.DryRun`; under dry-run print a per-provider `[dry-run] Would write state: <path>` preview (suppressed in JSON mode per cycle 187). Cycle 221 already gated the lock acquisition; cycle 226 closes the symmetric save gap. Regression test `TestRunRefresh_DryRunSkipsStateWrites` asserts probe still fires, NO state file lands on disk, and the preview text is emitted. (commit `bc2a39a`)

### Cycle 225 — `--from-repo=host.tld/user/repo` no longer misroutes to github.com

- [x] Real user-workflow bug. Pre-cycle-225 the URL expansion in `cloneRemoteRepo` was overzealous: any input without a scheme or `git@` prefix got `https://github.com/` prepended. So `hams apply --from-repo=gitlab.com/team/repo` silently became `https://github.com/gitlab.com/team/repo` — go-git then returned "Repository not found" against the wrong forge. Symmetric bug in `resolveClonePath`: 3-segment inputs fell into the defensive 2-segment fallback, producing `repo/team/repo` on disk (no host scoping, contradicting cycle 168's invariant). Fix: extract `expandRepoShorthand(repo)` mirroring `resolveClonePath`'s host-detection heuristic — first segment containing a dot is treated as a host (`https://<host>/<rest>`); otherwise GitHub shorthand. `resolveClonePath` now preserves all segments past a host-like first segment. New tests: `TestExpandRepoShorthand` (table-driven over 13 input shapes) + `TestExpandRepoShorthand_AgreesWithResolveClonePath` (invariant: URL form and clone path always reference the same host). (commit `aa6c585`)

### Cycle 224 — defaults/duti/git-config/git-clone CLI handlers acquire state lock

- [x] Closes the remaining gap from cycle 223 (which covered the 9 Package-class providers). Per cli-architecture/spec.md §"Lock file for single-writer enforcement", every provider install/remove (and any other mutating CLI handler) must acquire the single-writer lock. Cycle 224 wires `provider.AcquireMutationLockFromCfg` into the seven non-Package handlers: `defaults.handleWrite`, `defaults.handleDelete`, `duti.handleSet`, `git/git.go.doSet`, `git/git.go.doRemove`, `git/clone.go.handleAdd`, `git/clone.go.handleRemove`. Same pattern as cycle 223: acquire after dry-run preview, defer release before any runner exec / hamsfile or state write. Lock command labels are specific (e.g., `git-config set`, `duti set`) so the next caller hitting a conflict sees what's holding the lock. (commit `757b867`)

### Cycle 223 — Unify lock acquisition across runApply + 9 Package-class CLI handlers

- [x] runApply pre-dated the cycle-221 `provider.AcquireMutationLock` helper and held the lock via raw `state.NewLock + Acquire + Release`. Route it through the package-level `acquireMutationLock` seam so apply matches refresh's testability surface. Then sweep `apt/cargo/goinstall/homebrew/mas/npm/pnpm/uv/vscodeext` — every install/remove (and brew tap/untap) handler now acquires the lock before runner exec. Pre-cycle-223 a `hams cargo install ripgrep` could race with an in-flight `hams apply --only=cargo` and clobber `cargo.state.yaml` (both writers drove `sf.Save` on the same file). New `provider.AcquireMutationLockFromCfg` helper short-circuits to a no-op release under `--dry-run` (acquiring would write `.lock` and violate the zero-side-effects contract). 5+ unit tests cover happy path, conflict, stale reclaim, dry-run skip, nil-flags handling, and nil-cfg → ExitUsageError. (commit `31854a1`)

### Cycle 222 — `provider.AcquireMutationLock` shared helper

- [x] Pre-requisite for cycle 223. Per cli-architecture/spec.md the single-writer lock applies to every mutating CLI op, but only `runApply` acquired it — refresh and every CLI install/remove silently violated the contract. Extract the lock-acquisition logic into `provider.AcquireMutationLock(stateDir, cmdName)` returning a `(release, err)` pair. ExitLockError on conflict carries the standard "Remove .lock if previous run crashed" hint plus the holding PID/command/start-time from `state.Lock.Acquire`. Stale-PID locks are reclaimed transparently. 3 unit tests (happy path, conflict-with-live-holder, stale-reclaim). (commit `943eb67`)

### Cycle 221 — `runRefresh` acquires single-writer state lock

- [x] runRefresh wrote one state file per probed provider but never held the .lock — concurrent `hams refresh` runs could clobber each other (and concurrent `hams refresh` + `hams apply` raced on every per-provider state file). Wire `provider.AcquireMutationLock` into `runRefresh` via the new package-level `acquireMutationLock` seam (so DI-isolated tests like the save-failure-ordering scenario can stub it to a no-op without colliding with the real lock). Skip under `--dry-run`. 2 new regression tests (`TestRunRefresh_AcquiresMutationLock`, `TestRunRefresh_DryRunSkipsLock`). (commit `94d55d4`)
