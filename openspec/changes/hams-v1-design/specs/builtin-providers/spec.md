...
The apt provider SHALL wrap `apt install` and `apt remove` for Debian/Ubuntu-based Linux systems. It is Linux-only and requires sudo for all operations.
...
- `-y` SHALL be auto-injected on all `apt install` and `apt remove` commands to avoid interactive confirmation.
- `sudo` SHALL be auto-injected for all apt commands (apt requires root).

**Probe implementation:**

- `dpkg-query -W -f='${Package}\t${Version}\t${Status}\n'` -- parse for packages with status `install ok installed`.
- Alternative: `apt list --installed` (less reliable parsing).

**Apply flow:**

1. `sudo apt update` (once per apply session, not per package).
2. `sudo apt install -y <package>` for each missing package.

**Remove flow:**

1. `sudo apt remove -y <package>`.
...
| Subcommand | Behavior |
|------------|----------|
| `hams apt install <pkg>` | Install + record. |
| `hams apt remove <pkg>` | Remove + delete from Hamsfile. |
| `hams apt list` | Diff view. |
| `hams apt search <query>` | Passthrough to `apt search`. |
| `hams apt show <pkg>` | Passthrough to `apt show`. |
| Any other | Passthrough to `apt <subcommand>`. |

**LLM enrichment:**

- Source: `apt show <package>` provides `Description`, `Homepage`.

#### Scenario: Install an apt package

WHEN the user runs `hams apt install htop`
THEN the apt provider SHALL execute `sudo apt install -y htop`, record `{app: htop}` in `apt.hams.yaml`, and update state.

#### Scenario: Probe apt packages

WHEN the apt provider runs probe
THEN it SHALL execute `dpkg-query -W -f='${Package}\t${Version}\t${Status}\n'`, parse all packages with `install ok installed` status, and update state accordingly.
...
THEN the apt provider SHALL run `sudo apt update` exactly once at the beginning of its apply phase, not before each individual package install.
