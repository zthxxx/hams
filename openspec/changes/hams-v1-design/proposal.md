...
- **New CLI binary** (`hams`): Go static binary with urfave/cli v3 command routing, multi-level subcommands (`hams brew install`, `hams pnpm add`, `hams apply`, etc.).
...
- `cli-architecture`: urfave/cli bootstrap, command routing (`hams <global-flags> <provider> <verb> <args> --hams:flags -- <passthrough>`), lock file (PID+cmd), sudo management (once-at-startup), i18n (`LC_ALL`/`LANG` parsing), exit code semantics, error format (AI-agent friendly), `self-upgrade` command.
...
- **External dependencies**: urfave/cli v3, `hashicorp/go-plugin`, go-git, BubbleTea (TUI), `go-yaml` (comment-preserving YAML).
