...
- [x] **shared-abstraction-migration** (partial) — 2026-04-18 via `openspec/changes/2026-04-18-shared-abstraction-migration/`. Committed: (a) `internal/provider/package_dispatcher.go` ports `PackageInstaller` + `PackageDispatchOpts` + `AutoRecordInstall` / `AutoRecordRemove` from `/tmp/hams-loop/`; (b) `internal/provider/passthrough.go` adds `provider.Passthrough` + the shared `PassthroughExec` DI seam; (c) cargo is the reference migration — its `hamsfile.go` shrinks from 64 LOC of boilerplate to 4, `cargo.go` uses `baseprovider.LoadOrCreateHamsfile` + `baseprovider.EffectiveConfig`, and the `HandleCommand` default branch uses `provider.Passthrough`. 11 new unit tests cover the shared helpers; cargo's existing 13 tests pass unchanged. Deferred: migrating `goinstall`, `npm`, `pnpm`, `uv`, `mas`, `vscodeext`, `homebrew`, and `apt` follows the cargo template mechanically; tracked in the change folder's tasks.md §3.
